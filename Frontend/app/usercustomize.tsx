...
import * as SecureStore from "expo-secure-store"; // Corrected import
...
const AUTH_TOKEN_KEY = "authToken"; // Define a key for storing the auth token

// Define event category interface
...
const UserCustomize = () => {
  ...
  // Function to prepare image for upload
  ...
  // Handle continue button press - API integration
  const handleContinue = async () => {
    ...
    try {
      ...
      // Add each preference to form data (backend expects array)
      ...
      if (profileImage) {
        const imageFile = await prepareImageForUpload(profileImage);
        if (imageFile) {
      ...
      const response = await fetch(
        `${API_BASE_URL}/api/complete-attendee-profile`,
        {
          method: "POST",
          headers: {
            Authorization: `Bearer ${storedToken}`,
            // Note: Don't set Content-Type for multipart/form-data - the browser will set it with the boundary
          },
          body: formData,
        }
      );
  ...
  useEffect(() => {
    const fetchProfileData = async () => {
      try {
          ...
          if (response.ok) {
            ...
            if (profileData.country) {
              ...
              if (foundCountry) {
                ...
                // If city is available, try to find it
                if (profileData.city) {
                  const cities =
                    City.getCitiesOfCountry(foundCountry.isoCode) || [];
                  const foundCity = cities.find(
                    (c) =>
                      c.name.toLowerCase() === profileData.city.toLowerCase()
                  );
                  if (foundCity) {
            ...
            if (profileData.preferences && profileData.preferences.length > 0) {
              const updatedCategories = categories.map((cat) => ({
                ...cat,
                selected: profileData.preferences.includes(cat.name),
              }));
        ...
        // We don't show an error here as this might be a new user
      }
    };
  ...
  // Handle back button press - navigate to selectuser screen using expo-router
  const handleBack = () => {
